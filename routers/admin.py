...
def _append_audit_log(db: Session, *, action_type: AdminActionType, admin_user_id: int, details: str) -> AuditLog:
    record = AuditLog(
...
def _count_recent_alerts(db: Session, window_minutes: int = 60) -> int:
    ...
    unmatched_lpr_alerts = db.query(LprDetection).filter(
        LprDetection.review_status == "unmatched",
        LprDetection.detected_at >= cutoff
    ).count()

    denied_entry_alerts = db.query(EntryExitLog).filter(
...
def get_dashboard_summary(
    ...
    # Get latest occupancy count across all zones
    # This is a simplified sum of latest snapshots
    subq = db.query(
        OccupancySnapshot.zone_id,
        func.max(OccupancySnapshot.updated_at).label("max_updated")
    ).group_by(OccupancySnapshot.zone_id).subquery()

    occupied_count = db.query(func.sum(OccupancySnapshot.occupied_count)).join(
        subq,
        (OccupancySnapshot.zone_id == subq.c.zone_id) &
        (OccupancySnapshot.updated_at == subq.c.max_updated)
    ).scalar() or 0
    ...
    vehicles_inside = _count_vehicles_inside(db)
    
    unmatched_lpr_count = db.query(LprDetection).filter(
        LprDetection.review_status == "unmatched"
    ).count()
...
def resolve_unmatched_lpr(
    ...
    """
    detection = db.query(LprDetection).filter(LprDetection.id == payload.detection_id).first()
    if not detection:
        raise HTTPException(status_code=404, detail="LPR detection not found.")

    if payload.vehicle_id is not None:
        vehicle = db.query(Vehicle).filter(Vehicle.id == payload.vehicle_id, Vehicle.is_active == True).first()
        if not vehicle:
