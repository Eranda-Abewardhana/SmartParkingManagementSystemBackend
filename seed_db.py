    ...
    try:
        ...
        if db.query(UniversityMember).count() == 0:
            ...
            members = [
                ...
                UniversityMember(
                    university_id="UOC-SF-002",
                    full_name="Prof. Johnson",
                    email="staff2@uoc.edu",
                    official_role="staff",
                    is_active=True,
                ),
                UniversityMember(
                    university_id="UOC-ST-003",
                    full_name="Alice Wong",
                    email="student3@uoc.edu",
                    official_role="student",
                    is_active=True,
                ),
                UniversityMember(
                    university_id="UOC-SF-003",
                    full_name="Sarah Miller",
                    email="staff3@uoc.edu",
                    official_role="staff",
                    is_active=True,
                ),
            ]
        ...
        if db.query(Camera).count() == 0:
            ...
            cameras = [
                Camera(
                    ...
                    url="0",
                    zone_id=zones[0].id if len(zones) > 0 else None,
                    is_active=True,
                ),
                Camera(
                    name="Faculty Wing Camera",
                    url="0",
                    zone_id=zones[1].id if len(zones) > 1 else None,
                    is_active=True,
                ),
                Camera(
                    name="Visitor Lot Camera",
                    url="0",
        ...
        if db.query(Reservation).count() == 0:
            ...
            for i in range(10):
                ...
                slots = generate_slots(rows=4, cols=4)
