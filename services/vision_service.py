...
class VisionService:
    ...
    def load_models(cls):
        ...
        with cls._model_lock:
            if cls._plate_detector is None:
                try:
                    path = hf_hub_download(repo_id=cls.DETECTOR_REPO, filename="best.pt", token=cls.HF_TOKEN)
            ...
            if cls._ocr_model is None:
                try:
                    cls._ocr_processor = TrOCRProcessor.from_pretrained("microsoft/trocr-base-printed",
                                                                        token=cls.HF_TOKEN)
    ...
    @classmethod
    def analyze_parking_frame(cls, frame: np.ndarray, refresh_slots: bool = False, cam_id: int = 0) -> List[
        Dict[str, Any]]:
        cls.load_models()
        if frame is None or cls._plate_detector is None: return []

        # 1. Global Detection (Find everything)
        results = cls._plate_detector.predict(source=frame, conf=cls.DETECTOR_CONFIDENCE, imgsz=cls.DETECTOR_IMGSZ,
                                              verbose=False)
        ...
        for zone in zones:
            ...
            plates_in_zone = [p for p in all_detections if
                              zone['x'] <= (p['bbox'][0] + p['bbox'][2]) / 2 <= zone['x'] + zone['w']]
            ...
            if plates_in_zone:
                ...
                final_results.append({
                    ...
                    "x": best['bbox'][0], "y": best['bbox'][1], "w": best['bbox'][2] - best['bbox'][0],
                    "h": best['bbox'][3] - best['bbox'][1]
                })
            else:
                final_results.append(
                    {"slot_id": zone['id'], "occupied": False, "plate_number": None, "x": zone['x'], "y": 0,
                     "w": zone['w'], "h": h_f})
